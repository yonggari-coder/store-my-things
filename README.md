...
📍 BLE Beacon 기반 실내 정밀 탐색 솔루션 (Indoor Item Finder)

## 1. 프로젝트 개요

실내에서 빈번하게 발생하는 소형 귀중품(차키, 지갑, 리모컨 등)의 분실 문제를 해결하기 위한 BLE(Bluetooth Low Energy) 기반의 위치 추적 시스템입니다. GPS 수신이 불가능한 실내 환경의 한계를 극복하기 위해 iBeacon 표준 프로토콜을 활용하며, 스마트폰과 비콘 태그 간의 신호 세기(RSSI) 분석을 통해 미터 단위의 근거리 정밀 탐색 기능을 제공합니다.

본 프로젝트는 저전력·저비용 하드웨어와 고도화된 신호 필터링 알고리즘을 결합하여, 사용자가 직관적인 UI(레이더 방식)를 통해 물건의 위치를 파악하고 관리할 수 있는 사용자 경험을 설계하는 데 중점을 둡니다.
